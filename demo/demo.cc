...
void refresh(float progress)
{
    ...
    // 整体放大
    int pa = progress * 50;
    cout << "\33[1A"; // 终端光标向上移动行
    cout << "[" + string(pa, '=') + ">" + string(50 - pa, ' ') << "]  " << progress * 100 << "%" << endl;
    fflush(stdout); // 刷新缓冲区
}

void print_progress(int current, int total)
{
	float percentage = current / static_cast<float>(total);
    refresh(percentage);
}
...
int main(int argc, char* argv[])
{
	...
	aec_config.delay.use_external_delay_estimator = false;
